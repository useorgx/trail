...
parentPort.on('message', async ({ file, client }) => {
  ...
  try {
    ...
    const sess = {
      ...
      threads: r.threads.map((t) => decide({ id: t.id, origin: t.origin, kind: t.kind, title: t.title, ask: t.ask, notes: t.notes, moves: t.moves, backs: t.backs, status: t.status, claim: t.claim, subj: t.subj, errs: t.errs, parent: t.parent, t0: t.t0, t1: t.t1, feat: t.feat })),
    };
